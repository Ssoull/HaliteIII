...
/* void Dstar::initWithPosition(hlt::Position startPos, hlt::Position goalPos)
 * --------------------------
 * Init dstar with start and goal position
 * Use the default init at the end :D
 */
void Dstar::initWithPosition(hlt::Position startPos, hlt::Position goalPos) {
  init(startPos.x, startPos.y, goalPos.x, goalPos.y);
}
...
int Dstar::computeShortestPath() {
  ...
  while ((!openList.empty()) &&
    ...
    if (k++ > maxSteps) {
      hlt::log::log("[Dstar::computeShortestPath] At maxsteps");
      return -1;
    }
